...
const Introduct2 = () => {
    ...
    useEffect(() => {
        axios.get(`https://znt76d-8080.csb.app/introductions`)
            .then(data => setIntroducts(data.data))
    ...
    return (
        <div className='my-5 w-full flex justify-center min-h-fit'>
            {introducts.slice(3, 4).map((introduct, index) => (
                <div key={introduct.id} className={`w-2/5 flex justify-center h-50 bg-yellow-400 cursor-pointer relative not-lg:w-full not-lg:mx-2 ${close ? 'hidden' : ''}`}>
